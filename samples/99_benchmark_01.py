...
# **********************
duration = timeit.Timer('x = [normal_read(make) for make in makers]', globals=globals()).repeat(repeat=loops, number=loops)[0]
...
duration = timeit.Timer('y = [cached_read(make) for make in makers]', globals=globals()).repeat(repeat=loops, number=loops)[0]
print(f"{loops:,.0f}x read cube[{address}] caching '{caching_strategy}' in {duration:.3f} seconds, "
      f"{loops/duration * len(makers):,.0f} ops/sec, "
      f"{loops / duration * records_used * len(makers):,.0f} aggregations/sec, "
      f"{loops / duration * records_total * len(makers):,.0f} processed records/sec,")

duration = timeit.Timer('z = [df_read(make) for make in makers]', globals=globals()).repeat(repeat=loops, number=loops)[0]
