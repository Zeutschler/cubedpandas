...
class HybridDict(Iterable[Generic[T]]):
    ...
    def pop(self, item):
        """Removes a specific item at the given index from the list and returns the removed item."""
        if type(item) is int:
            # member by index
    ...
    def __setitem__(self, item, value) -> T:
        if type(item) is int:
            # member by index
            raise TypeError("Setting only supported for non int datatypes.")
        else:
            # member by key
            key = str(item).lower()
    ...
    def __repr__(self) -> str:
        # return pretty_str_formatter(HybridDict[T], self._list)
        if self._list:
            count = len(self._list)
            text = f"HybridDict({len(self._list)})[{', '.join([str(m) for m in self._list[:min(3, count)]])}"
            if count > 3:
                return text + f", ...]"
            return text + f"]"
        return "HybridDict(0)[]"
    ...
    def append(self, item: T) -> HybridDict[T]:
        ...
        if item is Iterable:
            ...
            self._dict[str(item).lower()] = item
        return self
    ...
    def index_of(self, item) -> int:
        """Returns the index of the first occurrence of an item in the HybridDict[T]."""
        return self._list.index(item)

    def to_list(self) -> list[T]:
        """Returns a list of the items conatained in the HybridDict[T]."""
        return copy(self._list)

    def as_text(self) -> list[str]:
        """Returns a list of the items contained in the HybridDict[T] as strings."""
    ...
    def delete_by_value(self, item):
        """Delete a specific value from the HybridDict[T]."""
        while item in self._list:
            self._list.remove(item)
        key = str(item).lower()
        if key in self._dict:
    ...
    def get(self, item, default=None) -> T:
        """Returns an item from the HybridDict[T]. If the member does not exist,
        then a default value will be returned."""
        if type(item) is int:
            # member by index
            if -1 < item < len(self._list):
                return self._list[item]
            else:
                return default
        else:
            # member by key, ...or even pattern search
            return self._dict.get(str(item).lower(), default)
    ...
    def filter(self, pattern: str) -> HybridDict[T]:
        ...
        """
        filtered = fnmatch.filter(self.keys, pattern.lower())
